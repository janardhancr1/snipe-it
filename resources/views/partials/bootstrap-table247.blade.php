...
<script nonce="{{ csrf_token() }}">
    ...
    function genericActionsFormatter(destination) {
        return function (value,row) {
            ...
            if (destination=='maintenances') {
                var dest = 'hardware247/maintenances';
            }
    ...
    function assetTagLinkFormatter(value, row) {
        alert(row.asset.id);
        return '<a href="{{ url('/') }}/hardware247/' + row.asset.id + '"> ' + row.asset.asset_tag + '</a>';
    }

    function assetNameLinkFormatter(value, row) {
        if ((row.asset) && (row.asset.name)) {
            return '<a href="{{ url('/') }}/hardware247/' + row.asset.id + '"> ' + row.asset.name + '</a>';
        }
    ...
    function assetCompanyFilterFormatter(value, row) {
        if (value) {
            return '<a href="{{ url('/') }}/hardware247/?company_id=' + row.id + '"> ' + value + '</a>';
        }
    }

    function assetCompanyObjFilterFormatter(value, row) {
        if ((row) && (row.company)) {
            return '<a href="{{ url('/') }}/hardware247/?company_id=' + row.company.id + '"> ' + row.company.name + '</a>';
        }
    }
    ...
    function orderNumberObjFilterFormatter(value, row) {
        if (value) {
            return '<a href="{{ url('/') }}/hardware247/?order_number=' + row.order_number + '"> ' + row.order_number + '</a>';
        }
    }
