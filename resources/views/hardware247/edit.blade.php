...
@extends('layouts/edit-form', [
    ...
    'helpPosition' => 'right',
    'formAction' => ($item) ? route('hardware247.update', ['hardware' => $item->id]) : route('hardware257.store'),
])
...
@section('inputFields')
    ...
    @include ('partials.forms.edit.department', ['translated_name' => trans('general.department'), 'fieldname' => 'department_id'])
  ...
  <div class="form-group {{ $errors->has('asset_tag') ? ' has-error' : '' }}">
    <label for="asset_tag" class="col-md-3 control-label">{{ trans('admin/hardware/form.tag') }}</label>
      ...
      @if  ($item->id)
          ...
          <input class="form-control" type="text" readonly name="asset_tags[1]" id="asset_tag" value="{{ Input::old('asset_tag', $item->asset_tag) }}" data-validation="required">
          </div>
      @else
          <!-- we are creating a new asset - let people use more than one asset tag -->
          <div class="col-md-7 col-sm-12{{  (\App\Helpers\Helper::checkIfRequired($item, 'asset_tag')) ? ' required' : '' }}">
              <input class="form-control" readonly type="text" name="asset_tags[1]" id="asset_tag" value="{{ Input::old('asset_tag', \App\Models\Asset::autoincrement_asset()) }}" data-validation="required">
          </div>
          <div class="col-md-2 col-sm-12 hide">
  ...
  @include ('partials.forms.edit.location-select247', ['translated_name' => trans('admin/hardware/form.checkout_to'), 'fieldname' => 'assigned_location', 'style' => 'display:none;', 'required' => 'false'])
  @endif
