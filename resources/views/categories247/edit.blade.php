@extends('layouts/edit-form', [
    ...
    'formAction' => ($item) ? route('categories247.update', ['category' => $item->id]) : route('categories247.store'),
])
...
<div class="form-group {{ $errors->has('category_code') ? ' has-error' : '' }}">
    <label for="category_code" class="col-md-3 control-label">{{ trans('general247.code') }}</label>
    <div class="col-md-7 col-sm-12{{  (\App\Helpers\Helper::checkIfRequired($item, 'category_code')) ? ' required' : '' }}">
        <input class="form-control" type="text" name="category_code" id="category_code" maxlength="2" value="{{ Input::old('category_code', $item->category_code) }}" {!!   (\App\Helpers\Helper::checkIfRequired($item, 'category_code')) ? ' data-validation="required"' : '' !!}>
        {!! $errors->first('category_code', '<span class="alert-msg"><i class="fa fa-times"></i> :message</span>') !!}
    </div>
</div>

<!-- ERP Category -->
<div class="form-group {{ $errors->has('erp_category_id') ? ' has-error' : '' }}">
    <label for="erp_category_id" class="col-md-3 control-label">{{ trans('general247.erpcategory') }}</label>
    <div class="col-md-7 required">
        {{ Form::select('erp_category_id', $erpcategories , Input::old('erp_category_id', $item->erp_category_id), array('class'=>'select2', 'style'=>'min-width:350px', $item->itemCount() > 0 ? 'disabled' : '')) }}
        {!! $errors->first('erp_category_id', '<span class="alert-msg"><i class="fa fa-times"></i> :message</span>') !!}
    </div>
</div>
